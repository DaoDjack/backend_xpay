...
class UserOperationController extends Controller{
    ...
    public function transfer(){
        ...
        $temoin = DB::select("select transaction_charges.*,min(min_limit) as min_limit,max(max_limit) as max_limit  from transaction_charges where slug = 'money_transfer' order by min_limit ");
        foreach($temoin[0] as $key=>$val)
        {
            $transferCharge->$key = $val;
        }
        $transferCharge = db_list("select transaction_charges.*  from transaction_charges where slug = 'money_transfer' order by min_limit ");
    ...
    public function transferMoney(Request $request){
        ...
        $tamount = $request->amount*$rate;
        $transferCharge = TransactionCharge::where('slug', 'money_transfer')->where('min_limit','<=', $amount)->where('max_limit','>=', $amount)->first();
        $transferCharge = db_row("select * from transaction_charges where min_limit<$tamount and max_limit>$tamount and slug = 'money_transfer'");

        if(empty($transferCharge))
        {
            $notify[] = ['error', 'Please amount out of transfer limit'];
            return back()->withNotify($notify)->withInput();
        }
        elseif ($request->amount < currencyConverter($transferCharge->min_limit, $rate) || $request->amount > currencyConverter($transferCharge->max_limit, $rate)) {
