...
class MoneyOutController extends Controller{
    ...
    public function moneyOut(){
        $pageTitle = "Money Out";
        $moneyOutCharge = db_list("select * from transaction_charges where slug = 'money_out_charge'");
    ...
    public function moneyOutConfirm(Request $request){
        ...
        $amount =$_REQUEST['amount']*$rate;
        $moneyOutCharge = TransactionCharge::where('slug', 'money_out_charge')->firstOrFail();
        $query = "select * from transaction_charges where slug='money_out_charge' and min_limit<$amount and max_limit>$amount";
        $moneyOutCharge = db_row($query);
        if(empty($moneyOutCharge))
        {
