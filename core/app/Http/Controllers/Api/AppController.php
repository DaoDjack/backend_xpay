...
class AppController extends Controller{
    ...
    public function querySetting(){
        ...
        global $return;
        $return['error']=1;
        if(empty($_REQUEST['ajx'])) $_REQUEST['ajx'] = "none";
       $file = 'jsons/'.$_REQUEST['ajx'].'.php';
        ...
        {
            if(!empty($_REQUEST['ajx'])) $return['message'] = "Le fichier que vous essayé d'executé a été supprimé ou deplacé";
        }
        //retour de l'information
        if($return['error']==1 and empty($return['message'])) $return['message']= "La requête n'a pas aboutie";
