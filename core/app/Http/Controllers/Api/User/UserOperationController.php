...
class UserOperationController extends Controller{
    ...
    public function transferMoney(Request $request){
        ...
        if($request->getway == "MOBILE MONEY"){
            
             //  Start process Transfert via mobile money
...
*/
        ...
        // Verification cinetpay
        
        
        $resultCheckBalance = $this->checkBalance($request->token, $request->amount);
        if($resultCheckBalance){
            $resultAddContact = $this->addNumberToCinetPay($request->token, $request->user);
            
             $data = json_decode($resultAddContact, true);
        
            if($data['code'] == 0  &&  $data['message'] == "OPERATION_SUCCES")
            {
            
            // Envoi d'argent via cinetpay
            
               $resultSendMoney = $this->SendMoney($request->token, $request->user, $request->amount);
                
             $dataSendMoney = json_decode($resultSendMoney, true);
             if($dataSendMoney['code'] == 0  &&  $dataSendMoney['message'] == "OPERATION_SUCCES")
            {
       ...
       if($userAction->verify_api_otp){
        ...
        $senderTrx->receiver_number	 =$request->user;
        $senderTrx->receiver_type = "USER";
        $senderTrx->trx = getTrx();
        $senderTrx->gateway = 'MOBILE MONEY';
        $senderTrx->save();
        ...
        // End Enregistrement dans la table

      }else{
            return response()->json([
            'remark'=>'money_transfer_reject',
            'status'=>'error',
            'message'=>['error'=>['Une erreur est surevenue lors du transfert']],
            ]); 
        }
    }  
            else{
           ...
           return response()->json([
            'remark'=>'money_transfer_rejett',
            'status'=>'error',
            'message'=>['error'=>['Fond insuffisant']],
        ]);
 ...
 // End process Transfert via mobile money
    ...
    // Fonction transaction cinetPay
   public function checkBalance($token, $amount) {
    $url = "https://client.cinetpay.com/v1/transfer/check/balance?code=0&token=$token&lang=fr";

    $response = file_get_contents($url);

    if ($response != false) {
        $responseCheckBalance = json_decode($response, true);

        if ($responseCheckBalance['message'] == "OPERATION_SUCCES") {
            $balanceCinetPay = $responseCheckBalance['data']['countryBalance']['CD']['available'];

            if ($balanceCinetPay > $amount) {
                return true;
            } else {
                return false;
            }
   ...
   public function addNumberToCinetPay($token, $number) {
        $curl = curl_init();
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL => 'https://client.cinetpay.com/v1/transfer/contact?lang=fr&token='.$token,
   ...
   public function SendMoney($token, $number, $amount)
   {
         $curl = curl_init();
            ...
            $params[] = [
              ...
              'phone' => $number,
              'amount' => $amount,
              'notify_url' => 'https://webhook.site/d777bad4-4870-4f65-9739-9ccdb0ffcb95'
              ];
            $data = array('data' => json_encode($params));

            curl_setopt_array($curl, array(
              CURLOPT_URL => 'https://client.cinetpay.com/v1/transfer/money/send/contact?token='.$token.'&transaction_id=trans001',
              CURLOPT_RETURNTRANSFER => true,
              CURLOPT_ENCODING => '',
              CURLOPT_MAXREDIRS => 10,
              CURLOPT_TIMEOUT => 0,
       ...
       /* if ($err) {
          ...
          print_r("Envoyer de l'argent: " .$result);
