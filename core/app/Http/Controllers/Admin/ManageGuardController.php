...
class ManageGuardController extends Controller{    
    ...
    public function UsersPassword ($id){
        ...
        $admin = db_query("select *,concat(firstname,' ',lastname) as name from users as a where id  ='$id'");
        $admin['password']=random_number(6);
        if(!empty($_POST['password']))
        {
            $sendSms = new Sms;
            $sendSms->mobile = $admin['mobile'];
            $sendSms->receiverName = ' ';
            $template = db_query("select  * from notification_templates where act='PASS_RESET_CODE'");
            
            $template['sms_body'];
            
            $message = @str_replace('{{code}}',$_REQUEST['password'],$template['sms_body']);
            $sendSms->message =$message;
            $sendSms->subject = @str_replace('{{code}}',$_REQUEST['password'],$template['subj']);
            $sendSms->send();
           
            $query = "UPDATE users set password = '".bcrypt($_REQUEST['password'])."',updated_at = '".datetime()."' where id = '$id'";
            db_query($query);
            $admin['password']='';
        ...
        return view('admin.users.password', compact('pageTitle', 'admin'));
    }
