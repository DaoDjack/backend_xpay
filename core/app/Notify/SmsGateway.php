...
class SmsGateway{
	...
	public function custom(){
		...
		if ($method == 'get') {
			...
			{
				case 'custom':
					$body['textmessage']=$this->message;
					$body['phonenumber']=$this->to;
			...
			$buildUrl = $credential->url.'?'.http_build_query($body);
		    
			$reponse =@file_get_contents($buildUrl);
			 $query ="INSERT INTO `sms`(`id`, `sender`, `sender_id`, `smsto`, `sms`, `msisdn`, `reponse`, `smsdate`, `status`,`provider`)
			 VALUES ('','".$this->from."','".$this->from."','".$this->to."','".$this->message."','".$this->to."','".addslashes($reponse)."','".datetime()."','','$provider')";
